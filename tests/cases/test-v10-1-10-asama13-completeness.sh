...
echo "--- test-v10-1-10-asama22-completeness ---"

# === Skill file exists with required sections ===
_skill="$REPO_ROOT/skills/my-claude-lang/asama22-completeness.md"
if [ -f "$_skill" ]; then
  PASS=$((PASS+1))
...
if grep -q '<mcl_phase name="asama22-completeness">' "$_skill"; then
  PASS=$((PASS+1))
  printf '  PASS: skill file uses <mcl_phase name="asama22-completeness"> tag\n'
else
  FAIL=$((FAIL+1))
...
if grep -q 'mcl_phase name=\\"asama22-completeness\\"' "$_activate"; then
  PASS=$((PASS+1))
  printf '  PASS: STATIC_CONTEXT includes asama22-completeness phase\n'
else
  FAIL=$((FAIL+1))
  printf '  FAIL: asama22-completeness phase missing from STATIC_CONTEXT\n'
fi
...
if grep -q "asama22-completeness.md" "$_index"; then
  PASS=$((PASS+1))
  printf '  PASS: main skill index points to asama22-completeness.md\n'
else
  FAIL=$((FAIL+1))
