...
phase_5_state_population_fallback() {
  phase_header "5 — Hook-level state population fallback (8.19.0)"

  # Real wrapper init + a transcript that includes an Engineering Brief
  # (the canonical English structured field set) AND `<mcl_state_emit>`
  # markers for ops/perf/ui-sub-phase. After mcl-stop.sh runs, state.json
  # MUST carry the populated fields — even though no skill prose Bash
  # was invoked. This is the deterministic fallback that closes the
  # 8.10-8.17 silent-skip-skill-prose gap.
  ...
  # Transcript: assistant turn with an Engineering Brief (English,
  # structured Task/Requirements/Context) + three state-emit markers.
  local transcript="$proj/transcript.jsonl"
  python3 -c '
import json, sys
out = sys.argv[1]
brief = """<details><summary>🔄 Engineering Brief [EN]</summary>
[MCL TRANSLATOR PASS — tr → EN]
Task: render paginated admin user table with role gating
Requirements: React + FastAPI + Postgres, role=admin, performance=fast
Success criteria: admin sees user list, non-admin gets 403
Context: greenfield project, Docker Compose local
</details>
...
with open(out, "w") as f:
    ...
    f.write(json.dumps({
        "type":"assistant",
        "message":{"role":"assistant","content":[{"type":"text","text": brief}]}
    }) + "\n")
' "$transcript"

  # Phase 1 currently — wrapper init left current_phase=1.
  # Drive mcl-stop.sh; the state-population block fires as part of
  # standard turn-end processing.
  local stop_input
  stop_input="$(python3 -c 'import json,sys; print(json.dumps({"transcript_path": sys.argv[1], "session_id": "e2e-5", "cwd": sys.argv[2]}))' "$transcript" "$proj")"
  ...
  # ASSERT 1: phase1_intent populated from brief (Task: line)
  local intent
  intent="$(state_get_field "$state_file" phase1_intent 2>/dev/null)"
  if printf '%s' "$intent" | grep -qF "render paginated admin user table"; then
    PASS=$((PASS+1))
    printf '  PASS: phase1_intent populated from brief Task: line\n'
  else
    FAIL=$((FAIL+1))
    printf '  FAIL: phase1_intent did not capture brief Task — got: %s\n' "$intent"
  fi

  # ASSERT 2: phase1_constraints populated
  local constraints
  constraints="$(state_get_field "$state_file" phase1_constraints 2>/dev/null)"
  if printf '%s' "$constraints" | grep -qF "React + FastAPI + Postgres"; then
    PASS=$((PASS+1))
    printf '  PASS: phase1_constraints populated from brief Requirements: line\n'
  else
    FAIL=$((FAIL+1))
    printf '  FAIL: phase1_constraints not populated — got: %s\n' "$constraints"
  fi

  # ASSERT 3: phase1_stack_declared inferred (3 tags expected)
  local stack
  stack="$(state_get_field "$state_file" phase1_stack_declared 2>/dev/null)"
  for tag in react-frontend python db-postgres; do
    if printf '%s' "$stack" | grep -qF "$tag"; then
      PASS=$((PASS+1))
      printf '  PASS: stack inferred → %s\n' "$tag"
    else
      FAIL=$((FAIL+1))
      printf '  FAIL: stack did not infer %s — got: %s\n' "$tag" "$stack"
    fi
  done

  # ASSERT 4: phase1_ops populated from marker
  local ops_dt
  ops_dt="$(python3 -c "
import json
d = json.load(open('$state_file'))
...
" 2>/dev/null)"
  assert_equals "phase1_ops.deployment_target=docker (from marker)" "$ops_dt" "docker"

  # ASSERT 5: phase1_perf populated from marker
  local perf_bt
  perf_bt="$(python3 -c "
import json
d = json.load(open('$state_file'))
...
" 2>/dev/null)"
  assert_equals "phase1_perf.budget_tier=strict (from marker)" "$perf_bt" "strict"

  # ASSERT 6: ui_sub_phase=UI_REVIEW from marker
  local ui
  ui="$(state_get_field "$state_file" ui_sub_phase 2>/dev/null)"
  assert_equals "ui_sub_phase=UI_REVIEW (from marker)" "$ui" '"UI_REVIEW"'

  # ASSERT 7: audit captures source=brief-parse for the population events
  if grep -q "phase1_state_populated | mcl-stop | source=brief-parse" "$audit_log"; then
    PASS=$((PASS+1))
    printf '  PASS: audit caller=mcl-stop, source=brief-parse — populate path tagged\n'
  else
    FAIL=$((FAIL+1))
    printf '  FAIL: phase1_state_populated audit with source=brief-parse missing\n'
    grep "phase1_state_populated" "$audit_log" | head -3 || true
  fi

  cleanup_test_dir "$proj"
}

phase_5b_idempotency() {
  phase_header "5b — Hook-level fallback idempotency (existing state preserved)"

  # If state.json already has phase1_intent populated (skill prose Bash
  # path or earlier fallback turn), the hook MUST NOT overwrite. This
  # test plants a sentinel value and verifies post-stop preservation.
  ...
  # Plant a sentinel that brief parse would otherwise overwrite.
  _e2e_write_state "$state_file" '{
    "phase1_intent": "PRE-EXISTING SENTINEL",
    "phase1_constraints": "PRE-EXISTING SENTINEL"
  }'
...
out = sys.argv[1]
brief = """<details><summary>🔄 Engineering Brief [EN]</summary>
Task: this should NOT overwrite the sentinel
Requirements: this should NOT overwrite either
Context: react fastapi
</details>"""
with open(out, "w") as f:
...
' "$transcript"
  ...
  local stop_input
  stop_input="$(python3 -c 'import json,sys; print(json.dumps({"transcript_path": sys.argv[1], "session_id": "e2e-5b", "cwd": sys.argv[2]}))' "$transcript" "$proj")"
  printf '%s' "$stop_input" \
    | CLAUDE_PROJECT_DIR="$proj" \
      MCL_STATE_DIR="$proj/.mcl" \
      MCL_REPO_PATH="$REPO_ROOT" \
  ...
  local intent
  intent="$(state_get_field "$state_file" phase1_intent 2>/dev/null)"
  assert_equals "phase1_intent NOT overwritten by hook fallback" "$intent" '"PRE-EXISTING SENTINEL"'

  local constraints
  constraints="$(state_get_field "$state_file" phase1_constraints 2>/dev/null)"
  assert_equals "phase1_constraints NOT overwritten by hook fallback" "$constraints" '"PRE-EXISTING SENTINEL"'
