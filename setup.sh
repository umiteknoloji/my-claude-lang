...
check_file "$HOOK_LIB_DST/mcl-spec-save.sh"
check_file "$HOOK_LIB_DST/mcl-stack-detect.sh"
...
if [ "$SMOKE_FAIL" = "0" ]; then
  echo "── Batch TDD (mandatory, since 5.12.0) ──"
  echo "TDD runs automatically on every Phase 4 execution. MCL resolves the"
  echo "test command in this order:"
  echo "  1. 'test_command' in .mcl/config.json (explicit override)"
  echo "  2. Framework auto-detect from project manifest:"
  echo "     package.json + scripts.test → npm test"
  echo "     pyproject.toml (pytest)     → pytest"
  echo "     Cargo.toml                  → cargo test"
  echo "     go.mod                      → go test ./..."
  echo "     pom.xml                     → mvn test"
  echo "     build.gradle(.kts)          → gradle test"
  echo "  3. If neither resolves, MCL asks you — reply 'none' to skip."
  echo ""
  echo "The resolved command runs in YOUR shell with NO sandbox. MCL does"
  echo "not review the command — audit any test_command entries yourself."
  echo ""
