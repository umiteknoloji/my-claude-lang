...
**Hook-First Architecture (9.1.0):** Tüm "model Bash zorunlu" path'leri "hook fallback" path'lerine taşındı. Skill prose Bash opt-in optimization olarak kalır (caller=skill-prose audit provenance, synchronous-write); yokluğunda hook her şeyi sessizce halleder. 5 path: `phase1_intent/constraints/stack_declared` (brief parse, 8.18.2), `precision-audit` audit (spec marker count + stack-detect → hook auto-emit), `phase1_ops/perf` (industry-default fill), `ui_sub_phase=UI_REVIEW` (dev-server-started + URL + prose cue heuristic), `phase5-verify` audit (14-dil localized header detection). Sonuç: kullanıcı sadece "Phase 1 sorusu → spec onayı → çalışan proje" akışını görür; "precision-audit", "skip-precision-audit", "block fired" mesajları UX'ten çıkar. Audit'te `source=hook-fallback / hook-default / hook-detection` etiketleri forensic provenance.
