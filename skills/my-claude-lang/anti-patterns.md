...
- ❌ Running Phase 5 (Verification Report) before Phase 4.5 is fully resolved — the report's Impact Analysis and must-test items must reflect the developer's Phase 4.5 decisions (skip / apply fix / make general rule). Emitting the report early produces a stale report
...
- ❌ Fabricating Phase 4.5 risks to make the section look thorough — only surface risks from an honest scan. If the code is clean, emit the single-sentence "No additional risks identified." fallback
...
- ❌ Listing ✅-compliant items in the Phase 5 Spec Compliance section — since MCL 5.3.0, Spec Compliance shows ONLY mismatches (⚠️/❌). If every MUST/SHOULD item is satisfied, emit a single sentence ("All MUST/SHOULD items comply.") — do NOT enumerate the satisfied items
...
- ❌ Including a Permission Summary section in Phase 5 — removed in MCL 5.2.0. The developer already saw and approved each permission at the harness prompt; restating adds no value. Phase 5 is 3 sections (Spec Compliance mismatches-only, Impact Analysis, must-test), not 4 or 5
