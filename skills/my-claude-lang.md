...
Every response MUST start with `🌐 MCL 6.5.0` on its own line. This tells the developer
...
`question` prefixed `MCL 6.5.0 | `. The Stop hook parses tool_use/tool_result
...
3. If ALL parameters clear → present summary as plain text, THEN call
   `AskUserQuestion({question: "MCL 6.5.0 | <localized-is-this-correct>",
...
6. Call `AskUserQuestion({question: "MCL 6.5.0 | <localized-spec-approval
...
Phase 3 `AskUserQuestion` call with prefix `MCL 6.5.0 | `.
