...
# MCL — Semantic Development Bridge (v10.1.14)
...
Every response MUST start with `🌐 MCL 10.1.14` on its own line.
...
`MCL 10.1.14 | `. The Stop hook parses tool_use/tool_result pairs to advance
MCL state.
...
3. If ALL parameters clear → present summary as plain text, THEN call
   `AskUserQuestion({question: "MCL 10.1.14 | <localized-is-this-correct>",
...
4. After the spec, explain in developer's language what it says.
   ...
   line.
5. Call `AskUserQuestion({question: "MCL 10.1.14 | <approval-prompt>",
...
When `ui_flow_active = false`, Aşama 6 is skipped entirely; Aşama 7
runs the standard code flow.
