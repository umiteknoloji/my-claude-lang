...
Every response MUST start with `🌐 MCL 8.0.3` on its own line. This tells the developer
...
`question` prefixed `MCL 8.0.3 | `. The Stop hook parses tool_use/tool_result
...
3. If ALL parameters clear → present summary as plain text, THEN call
   `AskUserQuestion({question: "MCL 8.0.3 | <localized-is-this-correct>",
...
6. Call `AskUserQuestion({question: "MCL 8.0.3 | <localized-spec-approval
...
Phase 3 `AskUserQuestion` call with prefix `MCL 8.0.3 | `.
...
Introduced in MCL 8.0.3. MCL, her tamamlanan task'ın Phase 5 sonunda `.mcl/project.md`
