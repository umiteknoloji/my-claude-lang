...
# MCL — Semantic Development Bridge (v10.1.8)
...
Every response MUST start with `🌐 MCL 10.1.8` on its own line.
...
`MCL 10.1.8 | `. The Stop hook parses tool_use/tool_result pairs to advance
MCL state.
...
3. If ALL parameters clear → present summary as plain text, THEN call
   `AskUserQuestion({question: "MCL 10.1.8 | <localized-is-this-correct>",
...
4. After the spec, explain in developer's language what it says.
   ...
   line.
5. Call `AskUserQuestion({question: "MCL 10.1.8 | <approval-prompt>",
