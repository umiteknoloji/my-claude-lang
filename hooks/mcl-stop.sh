...
if [ "$ASKQ_INTENT" = "spec-approve" ]; then
  ...
  if [ "$PARTIAL_NOW" = "true" ]; then
    ...
    # v13.1.0 — Hard enforce sıralılık: spec-approve advance hakkı için
    ...
    if [ -n "$_CHAIN_STATUS" ] && [ "$_CHAIN_STATUS" != "ok" ]; then
      # v13.1.0 — Auto-fill: developer's spec-approve onayı = retroactive
      # trust signal. Eksik chain audit'lerini "auto-fill-spec-approve"
      # markıyla emit et, advance'i fire ettir. v13.1.0'da burada block
