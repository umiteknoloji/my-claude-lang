...
# Pre-tool spec-approve advance JIT branch'i kaldırıldı (A8).
# Pseudocode "Stop hook fazları ilerletir" diyor — duplicate temizlendi.
# Stop hook turn sonu state advance + audit chain auto-fill yapar;
# pre-tool sadece mevcut state'e göre tool çağrılarını değerlendirir.
