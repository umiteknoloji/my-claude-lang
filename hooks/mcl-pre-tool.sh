...
' 2>/dev/null)"
    ...
    if [ "$JIT_INTENT" = "spec-approve" ] \
      ...
      if [ "$SPEC_APPROVED" = "true" ] && [ "$CURRENT_PHASE" -ge 5 ] 2>/dev/null \
        ...
        # v13.0.18 — Hard enforce sıralılık (parite Stop hook ile):
        ...
        if [ -n "$_JIT_CHAIN" ] && [ "$_JIT_CHAIN" != "ok" ]; then
          # v13.0.18 — Auto-fill missing chain audits. Developer's
        ...
        SPEC_APPROVED="$(mcl_state_get spec_approved 2>/dev/null)"
        PHASE_NAME="$(mcl_state_get phase_name 2>/dev/null)"
        fi  # v13.0.18 — closes _JIT_CHAIN incomplete check
      fi
...
if [ "$SPEC_APPROVED" = "true" ] && command -v python3 >/dev/null 2>&1; then
  # v13.0.18 — Removed `[ -z "$REASON" ]` guard. Layer B is the most
...
# v13.0.18 — fail-closed: if gate-spec has no allowed_tools field for
...
if allowed is None:
    print(f'deny:tool|tool={tool} state_phase={state_phase} active_phase={active_phase} reason=allowlist-undefined-fail-closed')
    sys.exit(0)
...
" 2>/dev/null)"
          ...
          # v13.0.18 — Always audit Layer B fire (debug visibility).
