...
# --- Loop-breaker helper (mirror of mcl-stop.sh, since v10.0.3) ---
...
# -------- Aşama 4 spec-presence enforcement (since v10.0.3) --------
# Every Edit/Write/MultiEdit/NotebookEdit tool call MUST be preceded by
# a 📋 Spec: block in the same assistant turn. No fast-path skipping —
# even small follow-up tweaks require a fresh spec describing the
# change. Loop-breaker after 3 strikes preserves v9.0.1 fail-open.
if [ -n "$TRANSCRIPT_PATH" ] && [ -f "$TRANSCRIPT_PATH" ] && command -v python3 >/dev/null 2>&1; then
  _SP_PRESENT="$(python3 - "$TRANSCRIPT_PATH" 2>/dev/null <<'PYEOF' || echo no
import json, re, sys
path = sys.argv[1]
...
# Concatenate all assistant text events AFTER the latest user message.
asst_text = "\n".join(t for r, t in events[last_user + 1:] if r == "assistant" and t)
print("yes" if SPEC_RE.search(asst_text) else "no")
PYEOF
)"
  if [ "$_SP_PRESENT" = "no" ]; then
    _SP_BLOCK_COUNT="$(_mcl_loop_breaker_count "spec-required-block" 2>/dev/null || echo 0)"
    if [ "${_SP_BLOCK_COUNT:-0}" -ge 3 ]; then
      mcl_audit_log "spec-required-loop-broken" "pre-tool" "tool=${TOOL_NAME} count=${_SP_BLOCK_COUNT} fail-open"
      command -v mcl_trace_append >/dev/null 2>&1 && mcl_trace_append spec_required_loop_broken "${_SP_BLOCK_COUNT}"
    else
      mcl_audit_log "spec-required-block" "pre-tool" "tool=${TOOL_NAME} count=${_SP_BLOCK_COUNT}"
      command -v mcl_trace_append >/dev/null 2>&1 && mcl_trace_append spec_required_block "${_SP_BLOCK_COUNT}"
      python3 -c '
import json, sys
count = sys.argv[1]
print(json.dumps({
    "decision": "block",
    "reason": "⚠️ MCL AŞAMA 4 SPEC-PRESENCE GATE — Bu turda hiçbir 📋 Spec: bloğu emit edilmedi. Edit/Write/MultiEdit/NotebookEdit çağırmadan ÖNCE bu turda görünür İngilizce 📋 Spec: bloğu yazmak ZORUNLU. Görev küçük olsa bile fast-path yok.\n\nBu turda yapman gereken:\n1. Edit aracını ÇAĞIRMA. Önce 📋 Spec: bloğu yaz (asistan metni olarak):\n   📋 Spec:\n   Changes: <bullet list of what files change and how>\n   Behavioral contract: <list>\n   Out of scope: <list>\n2. Sonra Edit/Write tool call yap.\n\nLoop-breaker: 3 üst üste skip → fail-open + audit warn. Şu anki sayı: " + count + "/3."
}))
' "$_SP_BLOCK_COUNT" 2>/dev/null
      exit 0
    fi
  fi
fi
