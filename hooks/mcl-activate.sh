...
' "$STATE_FILE" 2>/dev/null)"
  if [ -n "$_PR_RULES" ]; then
    ...
    if [ "$_PR_COMPLIANCE" = "1" ]; then
      PATTERN_RULES_NOTICE="${PATTERN_RULES_NOTICE}<mcl_audit name=\\\"pattern-compliance\\\">\\nPHASE 4.5 COMPLIANCE — check each Phase 4 file against the pattern rules above AND the root cause rule below.\\nReport violations as risk items: which file, which rule, what was found vs what is required.\\n\\nRoot Cause Lens: for each bug fix or behavioral change in Phase 4 code, ask — does this address the underlying cause, or only the visible symptom? Band-aids to look for: catching and swallowing errors, widening type constraints, special-casing inputs in tests, retrying without fixing the failure mode. Report any band-aid as a risk item.\\n</mcl_audit>\\n\\n"
    fi
  fi
fi
