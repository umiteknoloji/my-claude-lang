...
try:
  ...
  if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "$LAST_GATE_SESSION" ]; then
    ...
    if [ -f "$TRACE_LIB" ]; then
      ...
      # --- Session-boundary state resets (since 7.0.0) ---
      # drift_detected and partial_spec are task-scoped flags that must not
      # bleed into a new session started for an unrelated task. Clearing them
      # here prevents spurious drift/partial-spec warnings on session open.
      ...
      # --- Session log (since 7.0.0): create per-session .md diary ---
...
FULL_CONTEXT="${UPDATE_NOTICE}${SEMGREP_NOTICE}${PARTIAL_SPEC_NOTICE}${DRIFT_NOTICE}${PLUGIN_GATE_NOTICE}${STATIC_CONTEXT}"
