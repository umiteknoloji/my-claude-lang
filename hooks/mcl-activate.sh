...
# (drift_detected, partial_spec, phase_review_state) must be cleared
# BEFORE any notice computation reads them. Without this, DRIFT_NOTICE /
...
# Phase review enforcement notice (since 7.1.4). When the Stop hook set
...
# `hooks/mcl-pre-tool.sh` and the approved 0008 spec). The activate hook
# surfaces a one-turn DRIFT_NOTICE telling Claude to acknowledge in one
# line and proceed with the approved spec. The Stop hook auto-clears
# drift at the end of the warning turn (warn-once model, since 7.1.4) —
# no AskUserQuestion re-approval required.
DRIFT_NOTICE=""
if [ -f "$STATE_FILE" ] && command -v python3 >/dev/null 2>&1; then
...
' "$STATE_FILE" 2>/dev/null)"
  if [ -n "$DRIFT_INFO" ]; then
    ...
    DRIFT_NOTICE="<mcl_audit name=\\\"spec-drift\\\">\\nSPEC DRIFT DETECTED — the most recent emission produced a different hash (approved=${APPROVED_PREFIX}, current=${DRIFT_PREFIX}). WARN-ONCE MODEL (since 7.1.4): proceed with the APPROVED spec as-is. Do NOT re-emit the spec body. Do NOT call AskUserQuestion. Open with ONE localized line acknowledging drift (e.g. Turkish: 'Spec drift algılandı — onaylanan spec kullanılıyor.' / English: 'Spec drift detected — using approved spec.') then continue normally. The Stop hook auto-clears drift at the end of this turn.\\n</mcl_audit>\\n\\n"
  fi
fi

# Re-spec guard notice (since 7.1.4). When spec is approved and current_phase>=4,
...
# constraint `no-respec-after-approval` in STATIC_CONTEXT is the behavioral
...
try:
  ...
  if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "$LAST_GATE_SESSION" ]; then
    ...
    if [ -f "$TRACE_LIB" ]; then
      ...
      # --- Session log (since 7.1.4): create per-session .md diary ---
