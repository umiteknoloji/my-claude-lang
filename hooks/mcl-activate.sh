...
# Phase review enforcement notice (since 7.1.0). When the Stop hook set
...
try:
  ...
  if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "$LAST_GATE_SESSION" ]; then
    ...
    if [ -f "$TRACE_LIB" ]; then
      ...
      # --- Session-boundary state resets (since 7.1.0) ---
      # Task-scoped flags must not bleed into a new session. Clearing them
      # here prevents spurious drift/partial-spec/review warnings on open.
      ...
      # phase_review_state is task-scoped: reset on new session so a fresh
      # task starts without Phase 4.5 enforcement from the prior session.
      mcl_state_set phase_review_state null >/dev/null 2>&1 || true

      # --- Session log (since 7.1.0): create per-session .md diary ---
