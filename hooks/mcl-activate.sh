...
# Phase review enforcement notice (since 7.1.5). When the Stop hook set
...
# — mutating tools are no longer blocked by ${_BT}drift_detected=true${_BT} (see
# ${_BT}hooks/mcl-pre-tool.sh${_BT} and the approved 0008 spec). The activate hook
...
# drift at the end of the warning turn (warn-once model, since 7.1.5) —
...
' "$STATE_FILE" 2>/dev/null)"
  if [ -n "$DRIFT_INFO" ]; then
    ...
    DRIFT_NOTICE="<mcl_audit name=\\\"spec-drift\\\">\\nSPEC DRIFT DETECTED — the most recent emission produced a different hash (approved=${APPROVED_PREFIX}, current=${DRIFT_PREFIX}). WARN-ONCE MODEL (since 7.1.5): proceed with the APPROVED spec as-is. Do NOT re-emit the spec body. Do NOT call AskUserQuestion. Open with ONE localized line acknowledging drift (e.g. Turkish: 'Spec drift algılandı — onaylanan spec kullanılıyor.' / English: 'Spec drift detected — using approved spec.') then continue normally. The Stop hook auto-clears drift at the end of this turn.\\n</mcl_audit>\\n\\n"
  fi
fi

# Re-spec guard notice (since 7.1.5). When spec is approved and current_phase>=4,
...
try:
  ...
  if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "$LAST_GATE_SESSION" ]; then
    ...
    if [ -f "$TRACE_LIB" ]; then
      ...
      # --- Session log (since 7.1.5): create per-session .md diary ---
