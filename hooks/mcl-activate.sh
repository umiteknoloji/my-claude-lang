...
# Schema migration — runs on every UserPromptSubmit. Idempotent: if the
# state file is already on the current schema, nothing happens. If it's
# on an older schema (v1 or v2), `mcl_state_init` rewrites it to v3 and
# emits a `state-migrated-10.0.0` audit event with a backup at
# `.mcl/state.json.backup.pre-v3`.
_AC_STATE_LIB="$(dirname "$0")/lib/mcl-state.sh"
if [ -f "$_AC_STATE_LIB" ]; then
  # shellcheck source=hooks/lib/mcl-state.sh
  . "$_AC_STATE_LIB" 2>/dev/null && mcl_state_init 2>/dev/null || true
fi
...
# Installed version is read from the VERSION file shipped alongside the
# hooks. Resolves at runtime — no install-time substitution required, no
# stale literal in the script body. mcl-activate.sh sits at
# `<MCL_HOME>/lib/hooks/mcl-activate.sh`; VERSION is one level up.
_MCL_HOOK_DIR_VS="$(cd "$(dirname "$0")" 2>/dev/null && pwd)"
INSTALLED_VERSION="$(cat "$_MCL_HOOK_DIR_VS/../VERSION" 2>/dev/null \
  | tr -d '[:space:]')"
...
if [ "$PROMPT_NORM" = "/mcl-restart" ]; then
  ...
  if [ -f "$_STATE_LIB" ]; then
    ...
    # re-promote design_approved=true using an old approve askq still
    # living in the same-session transcript, defeating the restart.
  ...
  cat <<RESTART_OUTPUT
{
  "hookSpecificOutput": {
    "hookEventName": "UserPromptSubmit",
    "additionalContext": "<mcl_core>\nMCL_RESTART_MODE — the developer typed the literal keyword ${_BT}/mcl-restart${_BT}. SKIP the entire MCL pipeline. Do NOT run Phase 1/Phase 3/Phase 4/Phase 5/Phase 6. Do NOT ask clarifying questions. Do NOT emit a spec block.\n\nAll MCL phase state has been reset to defaults (current_phase=1, design_approved=false, is_ui_project=false). Respond ONLY in the developer's detected language (default Turkish if unknown):\n\n1. Start the response with the banner ${_BT}🌐 MCL ${INSTALLED_VERSION} — mcl-restart${_BT}.\n2. Confirm in the developer's language that all phase state has been cleared and MCL is ready for a new task.\n3. Do NOT start Phase 1 automatically — just confirm the reset and wait for the developer's next message.\n\nSTOP RULE: no clarifying questions. ${_BT}/mcl-restart${_BT} is unambiguous — confirm the reset.\n</mcl_core>"
  }
}
RESTART_OUTPUT
  exit 0
...
# Substitute __MCL_VERSION__ placeholders left in the literal heredoc.
# The heredoc delimiter is single-quoted (preserves all $/backtick/paren
# escape sequences inside the JSON-encoded body); this pass restores
# version refs after the body lands intact. Cf. INSTALLED_VERSION read
# from VERSION at the top of this file.
STATIC_CONTEXT="${STATIC_CONTEXT//__MCL_VERSION__/$INSTALLED_VERSION}"
...
try:
  ...
  if [ -n "$_EARLY_SESSION_ID" ] && [ "$_EARLY_SESSION_ID" != "$_EARLY_LAST_SESSION" ]; then
    ...
    # 10.0.0: phase_review_state is a transient v2 field — left absent in v3
    # default state. Phase 4 dialog tracking lives in mcl-stop.sh transient
    # writes only; cross-session reset not required.
...
# the first Phase 4 scan in this session runs against a warm cache.
#
...
if [ -f "$SEMGREP_HELPER" ]; then
  ...
  case "$SEMGREP_RC" in
    2)
      ...
      # need the install hint mid-Phase-1. Phase 4 falls back to its
    ...
    0)
      ...
      # so Phase 4's first scan hits warm rules. No prose needed.
...
# and Claude must run Phase 4 before answering anything else.
PHASE_REVIEW_NOTICE=""
if [ -f "$STATE_FILE" ] && command -v python3 >/dev/null 2>&1; then
...
' "$STATE_FILE" 2>/dev/null)"
  if [ "$_PR_STATE_VAL" = "pending" ]; then
    PHASE_REVIEW_NOTICE="<mcl_audit name=\\\"phase-review-required\\\">\\nPHASE REVIEW MANDATORY BLOCK — Phase 4 code was written in a prior turn but Phase 4 Risk Review has NOT been started. The developer's current message must NOT be answered yet.\\n\\nBefore responding to ANYTHING the developer just said, you MUST:\\n1. Acknowledge in the developer's detected language (one sentence) that Phase 4 must run before you can continue.\\n2. Immediately start Phase 4 Risk Review: review ALL code written in Phase 4 for security vulnerabilities, performance bottlenecks, edge cases, data integrity, race conditions, and regression surfaces.\\n3. Present ONE risk at a time via AskUserQuestion with prefix ${_BT}MCL ${INSTALLED_VERSION} | ${_BT}.\\n4. After ALL Phase 4 risks are resolved → run Phase 4 (impact lens) Impact Review.\\n5. After Phase 4 (impact lens) → run Phase 5 Verification Report.\\n6. Only AFTER Phase 5 is complete → answer the developer's original message.\\n\\nThis is MANDATORY. The developer's message is queued — not ignored — but Phase 4/5 take priority.\\n</mcl_audit>\\n\\n"
  ...
  if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "$LAST_GATE_SESSION" ]; then
    ...
    if [ -f "$TRACE_LIB" ]; then
      ...
      if [ -f "$STACK_DETECT_LIB" ]; then
        ...
        # is_ui_project self-heal (since 10.0.0). If detection finds a UI
        # project but `is_ui_project` is still false (e.g. detection ran
        # late, or migrated state did not flag it), set it now so the
        # Phase 1 summary-confirm transition routes through Phase 2
        # DESIGN_REVIEW. Idempotent.
        if [ "$UI_CAPABLE" = "true" ]; then
          SH_IS_UI="$(mcl_state_get is_ui_project 2>/dev/null)"
          if [ "$SH_IS_UI" != "true" ]; then
            mcl_state_set is_ui_project true >/dev/null 2>&1 || true
            mcl_audit_log "is-ui-project-set" "mcl-activate.sh" "auto-detected"
            mcl_trace_append is_ui_project_set true
          fi
        fi
      fi
    fi
...
# Also adds a compliance check directive for Phase 4.
...
if [ "$_SD_PHASE" = "3" ]; then
  SCOPE_DISCIPLINE_NOTICE="<mcl_audit name=\\\"scope-discipline\\\">\\nSCOPE DISCIPLINE — three hard rules for Phase 3 (IMPLEMENTATION):\\n\\nRule 1 — SPEC-ONLY: Implement ONLY what the spec's MUST/SHOULD items explicitly require. FORBIDDEN without exception: performance improvements not in spec, refactors \\\"while I'm here\\\", style fixes, extra tests beyond acceptance criteria, API additions \\\"for future use\\\", removing unrelated dead code. If you notice something worth fixing: record it as a Phase 4 risk item — do NOT fix it now.\\n\\nRule 2 — FILE SCOPE: Only write/edit files the spec's Technical Approach references. If scope_paths is set the pre-tool hook blocks unlisted files. If it is empty: before touching any file verify it is spec-referenced — if unsure, surface it as a risk rather than silently editing.\\n\\nRule 3 — ROOT CAUSE: Fix the cause, not the symptom. If a patch only makes the test pass without addressing why it failed, surface it as a Phase 4 risk item instead of committing it.\\n</mcl_audit>\\n\\n"
fi
...
# ATOMIC_COMMIT_NOTICE fires on Phase 4/5 (pr_state=running) — also once.
ROLLBACK_NOTICE=""
...
# Claude must fix failing tests BEFORE starting Phase 4 risk dialog.
...
# Plugin dispatch audit — fires when Phase 4 is running to check whether
...
if [ -f "$_DISPATCH_AUDIT_LIB" ] && [ -f "$STATE_FILE" ] && command -v python3 >/dev/null 2>&1; then
  ...
  if [ "$_DA_REVIEW_STATE" = "running" ]; then
    ...
    if [ -n "$_DA_MISSED" ]; then
      ...
      PLUGIN_MISS_NOTICE="<mcl_audit name=\\\"plugin-dispatch-gap\\\">\\nPLUGIN DISPATCH GAP — Phase 4 is running but the following required plugins have NOT been dispatched yet: ${_DA_MISSED_ESC}.\\n\\nYou MUST dispatch each missing plugin as a Task sub-agent BEFORE writing any Phase 4 (impact lens) or Phase 5 content:\\n- ${_BT}code-review${_BT}: call Task with subagent_type=${_BT}pr-review-toolkit:code-reviewer${_BT} (or equivalent code-review sub-agent).\\n- ${_BT}semgrep${_BT}: run ${_BT}bash hooks/lib/mcl-semgrep.sh scan <files>${_BT} via Bash tool.\\nAfter dispatching, this notice will automatically clear on the next turn. Do NOT proceed to Phase 4 (impact lens) until it clears.\\n</mcl_audit>\\n\\n"
