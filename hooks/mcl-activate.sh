...
' "$STATE_FILE" 2>/dev/null)"
  ...
  if [ "$_UF_ACTIVE_V" = "True" ] || [ "$_UF_ACTIVE_V" = "true" ]; then
    if [ "${_UF_PHASE:-0}" -ge 4 ] 2>/dev/null && [ "$_UF_REVIEWED" != "True" ] && [ "$_UF_REVIEWED" != "true" ]; then
      UI_FLOW_NOTICE="<mcl_audit name=\\\"ui-flow-required\\\">\\nUI FLOW MANDATORY — A UI surface was detected in this project. Aşama 7 MUST follow the 4a/4b/4c sub-phase sequence. DO NOT write backend code or run Aşama 8/10/5 until Aşama 6b UI_REVIEW is complete.\\n\\nCurrent ui_sub_phase: ${_UF_USP:-BUILD_UI}. ui_reviewed: false.\\n\\nRequired sequence:\\n1. Aşama 6a BUILD_UI — build the full frontend UI with dummy/mock data only. Write build-tool config files FIRST (vite.config.*, tailwind.config.*, tsconfig.json, postcss.config.*) — the project must be runnable before asking for approval. NO real API calls. When done: run npm install if needed, start the dev server in background, auto-open in browser.\\n2. Aşama 6b UI_REVIEW — STOP. Present an AskUserQuestion for UI approval with prefix ${_BT}MCL ${INSTALLED_VERSION} | ${_BT}. Options: approve UI / request revisions. Do NOT proceed to backend until developer approves.\\n3. Aşama 6c BACKEND — only after UI approval: wire real API calls, data layer, async operations.\\n\\nSub-agents dispatched during Aşama 6a MUST build UI only — no backend wiring. Aşama 6b AskUserQuestion MUST run in the main MCL session, not a sub-agent.\\n</mcl_audit>\\n\\n"
    fi
  fi
fi
