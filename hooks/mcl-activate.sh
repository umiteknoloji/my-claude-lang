...
# Phase review enforcement notice (since 7.1.3). When the Stop hook set
...
# drift at the end of the warning turn (warn-once model, since 7.1.3) —
...
try:
  ...
  if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "$LAST_GATE_SESSION" ]; then
    ...
    if [ -f "$TRACE_LIB" ]; then
      ...
      # --- Session log (since 7.1.3): create per-session .md diary ---
...
FULL_CONTEXT="${UPDATE_NOTICE}${SEMGREP_NOTICE}${PARTIAL_SPEC_NOTICE}${PHASE_REVIEW_NOTICE}${DRIFT_NOTICE}${PLUGIN_GATE_NOTICE}${STATIC_CONTEXT}"
