...
# Phase review enforcement notice (since 7.1.2). When the Stop hook set
...
# drift at the end of the warning turn (warn-once model, since 7.1.2) —
...
try:
  ...
  if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "$LAST_GATE_SESSION" ]; then
    ...
    if [ -f "$TRACE_LIB" ]; then
      ...
      # --- Session log (since 7.1.2): create per-session .md diary ---
