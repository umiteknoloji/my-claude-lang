...
# Phase review enforcement notice (since 7.1.7). When the Stop hook set
...
# Re-spec guard notice (since 7.1.7). When spec is approved and current_phase>=4,
...
try:
  ...
  if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "$LAST_GATE_SESSION" ]; then
    ...
    if [ -f "$TRACE_LIB" ]; then
      ...
      # --- Session log (since 7.1.7): create per-session .md diary ---
