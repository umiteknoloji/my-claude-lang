...
' "$STATE_FILE" 2>/dev/null)"
  if [ -n "$_PR_RULES" ]; then
    ...
    if [ "$_PR_COMPLIANCE" = "1" ]; then
      PATTERN_RULES_NOTICE="${PATTERN_RULES_NOTICE}<mcl_audit name=\\\"pattern-compliance\\\">\\nPHASE 4.5 COMPLIANCE — check each Phase 4 file against the pattern rules above AND the root cause rule below.\\nReport violations as risk items: which file, which rule, what was found vs what is required.\\n\\nRoot Cause Chain: for each bug fix or behavioral change in Phase 4 code, apply a multi-level chain — do NOT stop at the first cause found. For each cause identified, ask again: does THIS cause have a deeper root cause? Descend until you reach a structural cause with no further parent. Band-aids to look for at any level: catching and swallowing errors, widening type constraints, special-casing inputs in tests, retrying without fixing the failure mode. Report the deepest cause as the risk item, not the surface symptom. Skipping this chain is forbidden — Phase 4.5 is not complete until the chain bottoms out.\\n</mcl_audit>\\n\\n"
    fi
  fi
fi
