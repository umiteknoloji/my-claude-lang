...
' 2>/dev/null)"
    PATTERN_MATCHING_NOTICE="<mcl_audit name=\"phase-3.5-pattern-scan\">\nPHASE 3.5 — PATTERN SCAN REQUIRED (one-time, this turn only)\n\nBefore writing any code this session, read the existing sibling files below and extract the codebase's established patterns. This turn: READ ONLY — no file writes.\n\nFiles to read:\n${_PM_LIST}\n\nFor each file, note:\n- Import style (named/default, relative vs absolute paths)\n- Naming conventions (camelCase/snake_case/PascalCase for files, functions, types)\n- Error handling pattern (try/catch, Result type, custom error classes)\n- Test structure (describe/it blocks, fixture setup, assertion style)\n- TypeScript conventions (interface vs type, generic usage, strict null patterns)\n- Export style (named vs default exports)\n\nAfter reading, write a 3-5 line PATTERN SUMMARY (no code blocks). Then end the turn — writes unlock automatically on the next turn.\n</mcl_audit>\n\n"
    mcl_audit_log "pattern-matching-notice" "mcl-activate.sh" "shown"
  fi
fi

# Rollback checkpoint notice — shown every Phase 4 turn so the developer
# always knows the pre-spec HEAD SHA and the exact reset command.
ROLLBACK_NOTICE=""
...
try:
    ...
    # ATOMIC_COMMIT hint when Phase 4.5 / 4.6 / 5 is running (pr_state=running)
    atomic = "1" if pr_state == "running" else "0"
    print(f"{sha}|{scope_str}|{spec_hash}|{atomic}")
except Exception:
    print("")
' "$STATE_FILE" 2>/dev/null)"
...
FULL_CONTEXT="${UPDATE_NOTICE}${SEMGREP_NOTICE}${PROJECT_MEMORY_NOTICE}${PROACTIVE_NOTICE}${PARTIAL_SPEC_NOTICE}${PATTERN_MATCHING_NOTICE}${ROLLBACK_NOTICE}${ATOMIC_COMMIT_NOTICE}${REGRESSION_BLOCK_NOTICE}${PHASE_REVIEW_NOTICE}${RESPEC_GUARD_NOTICE}${PLUGIN_GATE_NOTICE}${UI_FLOW_NOTICE}${PLUGIN_MISS_NOTICE}${STATIC_CONTEXT}"
