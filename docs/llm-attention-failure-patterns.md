...
| 4 | **Aşama atlama / paralel koşma** — Faz 1 bitmeden Faz 3'e atlar, aynı anda birden fazla iş başlatır. | Allowlist; mevcut fazın tool'u dışında her şey reddedilsin. | Layer B Phase Allowlist (v13.0.9) — gate-spec.json `allowed_tools` + `denied_paths` + PreToolUse phase enforcement. **v13.1.0 always-fire**: REASON koşulu kaldırıldı, fail-closed (allowed_tools field yoksa DENY), debug audit her fire'da. **v13.0.16 persuasion manifesto** + **v13.0.17 spec-approve chain auto-fill** pre-hoc steering ekler. | ✅ |
