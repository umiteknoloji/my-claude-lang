...
- **Opt-in `/mycl` session trigger** (`hooks/lib/activation.py`,
  yeni). MyCL artık her promptta otomatik aktif değil; oturum
  başına bir kez `/mycl` (veya `/mycl <prompt>`) yazılana kadar tüm
  hook'lar no-op kalır — banner yok, deny yok, audit yok, snapshot
  yok. Trigger sonrası session_id `.mycl/active_session.txt`'e
  yazılır; aynı oturumda sonraki promptlar trigger gerektirmez.
  Yeni Claude Code oturumu yeni ID'ye sahip olduğu için pasif
  başlar (oturum-geçici kapsam). Trigger prompt'tan sıyrılır;
  sıyrılmış mesaj `<mycl_activation_note>` ile model'e iletilir.
- **`MYCL_TEST_FORCE_ACTIVE` env** — `activation.is_session_active`
  test ortamında aktivasyonu bypass eder; `tests/conftest.py`
  autouse fixture'ı tüm mevcut hook test'lerini koruyarak yeni
  davranışa geçişi sağlar.
- **`tests/test_activation.py`** (14 test) — extract_trigger
  matrix, activate/is_active roundtrip, oturum-geçici semantik,
  force-active env bypass.
- **`tests/test_activate.py`** opt-in eklemeleri (5 test) — hook
  pasif iken no-op, `/mycl` ile aktivasyon notu, çıplak `/mycl`
  davranışı, aynı session aktif kalır, farklı session_id pasif.

### Değişen / Changed

- Beş hook (`activate`, `pre_tool`, `post_tool`, `stop`,
  `pre_compact`) `_is_self_project` guard'ından hemen sonra
  `activation.is_session_active` guard'ı eklendi; aktif değilse
  her hook erken çıkar.
- README.md + README.tr.md — "Aktivasyon (1.0.5+)" bölümü;
  opt-in modelinin gerekçesi, kullanım, oturum-geçici semantik
  açıklaması.
- plugin.json — 1.0.3 → 1.0.5 sürüm bump (1.0.4'te plugin.json
  yanlışlıkla 1.0.3'te bırakılmıştı), description "opt-in"
  vurgusu, keywords'e `opt-in` + `slash-command` eklendi.
- 568 → 587 test (+19: 14 yeni test_activation + 5 yeni
  test_activate opt-in).
...
- **H-1: MUST extractor 0 öğe döndü** — `spec_detect.py`
  `_MUST_SECTION_RE` slash-joined "MUST / SHOULD" başlığını
  yakalayacak şekilde genişletildi; yeni `_MUST_INLINE_LIST_RE` ve
  `_SHOULD_INLINE_LIST_RE` list-marker'sız girintili `MUST_N text`
  formatını okur. Yol 1 (section + liste işareti) boş çıkarsa Yol 2
  (inline) fallback devreye girer.
- **H-2: Düz "onay" metni `spec_approved`'u set etmedi** —
  `stop.py::_detect_askq_intent` AskUserQuestion structured
  yanıtı yoksa son user mesajını `askq.classify` ile sınıflandırır;
  fallback dal `ambiguous` döndürürse None yorumlar.
- **H-3: Aşama 1'de AskUserQuestion deny** — `gate_spec.json`
  Aşama 1 `allowed_tools` listesine `AskUserQuestion` eklendi.
- **H-4: Banner "MyCL 0.0.0" eski kurulum tespiti** —
  `activate.py` banner'dan sonra VERSION major == "0" ise
  `<mycl_stale_version_warning>` emit eder.
- **H-5: Spec-approval block kurtarma yönlendirmesi** —
  `bilingual_messages.json` `spec_approval_block` mesajına
  oturum içi kurtarma ipucu eklendi.
- **H-6: Spec format dokümantasyonu** —
  `skills/mycl/asama04-spec.md` "Kabul edilen MUST/SHOULD öğe
  formatları" bölümü; üç format örnek (list-marker'lı, girintili,
  karma).

### Test

- 556 → 568 test (+12: extractor, askq fallback, stale-version
  uyarısı).
